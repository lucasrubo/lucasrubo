...
function snapToGrid(pos: { x: number; y: number }): { x: number; y: number } {
	return {
		x: Math.max(
			0,
			GRID.x + Math.round((pos.x - GRID.x) / GRID.stepX) * GRID.stepX,
		),
		y: Math.max(
			0,
			GRID.y + Math.round((pos.y - GRID.y) / GRID.stepY) * GRID.stepY,
		),
	};
}
...
function DesktopInner() {
	...
	const handleIconOpen = (
		...
		if (appId === "github") {
			window.open("https://github.com/lucasrubo", "_blank");
			return;
		}
		if (appId === "linkedin") {
			window.open("https://linkedin.com/in/lucas-rubo", "_blank");
			return;
		}
		if (appId === "whatsapp") {
			window.open("https://wa.me/5519994019804", "_blank");
			return;
		}
		if (appId === "instagram") {
			window.open("https://instagram.com/lucas.rubo", "_blank");
			return;
		}
				...
				{ALL_ICONS.map((icon) => {
					...
					return (
						<DesktopIcon
							...
							position={pos}
							onOpen={(clickPos) =>
								handleIconOpen(icon.appId, icon.label, clickPos)
							}
...
function CtxItem({
	...
	return (
		...
		>
			<span>{label}</span>
			{shortcut && (
				<span className="text-white/30 font-mono text-[10px]">{shortcut}</span>
			)}
		</button>
	);
}
