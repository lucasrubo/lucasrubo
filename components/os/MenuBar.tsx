...
export default function MenuBar() {
	...
	return (
		<header className="h-9 bg-[#1a1815] flex items-center px-3 gap-1 select-none shrink-0 z-50">
			...
			>
				<LRMark />
				<span className="text-white/85 text-[13px] font-semibold tracking-tight">
					AprixOS
				</span>
			</button>
			...
			<div className="flex items-center gap-1">
				...
				>
					<span
						className={`text-[11px] font-bold transition-colors ${locale === "en" ? "text-white" : "text-white/30"}`}
					>
						EN
					</span>
					<span className="text-white/20 text-[10px] mx-0.5">·</span>
					<span
						className={`text-[11px] font-bold transition-colors ${locale === "ptBR" ? "text-white" : "text-white/30"}`}
					>
						PT
					</span>
				</button>
