...
import { LEFT_ICONS, MIDDLE_ICONS, RIGHT_ICONS, type DesktopIconDef } from "@/lib/desktopIcons";
...
function MobileIconCell({ icon, onTap }: MobileIconCellProps) {
  const touchRef = useRef<{ moved: boolean; startX: number; startY: number }>({ moved: false, startX: 0, startY: 0 });

  return (
    <button
      className="mobile-icon-tap flex flex-col items-center gap-1.5 select-none focus:outline-none"
      onTouchStart={(e) => {
        touchRef.current = { moved: false, startX: e.touches[0].clientX, startY: e.touches[0].clientY };
      }}
      onTouchMove={(e) => {
...
export default function MobileHomeScreen() {
  ...
  const [openApp, setOpenApp] = useState<{ appId: string; title: string } | null>(
    { appId: "welcome", title: "welcome.app" }
  );
      ...
      <div className="flex-1 overflow-hidden">
        ...
        >
          {PAGES.map((icons, pageIndex) => (
            <div
              key={pageIndex}
              style={{ scrollSnapAlign: "start", flexShrink: 0, width: "100vw" }}
            >
